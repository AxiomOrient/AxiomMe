...
impl AxiomMe {
    ...
    pub(super) fn execute_eval_cases(
        ...
        for case in query_cases {
            ...
            let case_passed =
                case.expected_top_uri.is_some() && case.expected_top_uri == actual_top_uri;
            if case_passed {
                passed += 1;
            } else {
                failed += 1;
        ...
        let top1_accuracy = if query_cases.is_empty() {
            ...
            passed as f32 / query_cases.len() as f32
        };
    ...
    fn collect_required_failure_bucket_probes(
        ...
        let probe_root =
            std::env::temp_dir().join(format!("axiomme-eval-probe-{}", uuid::Uuid::new_v4()));
        ...
        let output = (|| -> Result<HashMap<String, usize>> {
            ...
            if !filter_ok {
                *buckets.entry("filter_ignored".to_string()).or_insert(0) += 1;
            }
            ...
            if !relation_ok {
                *buckets.entry("relation_missing".to_string()).or_insert(0) += 1;
            }
            ...
            let intent_ok = serde_json::from_value::<QueryPlan>(intent.query_plan.clone())
                .map(|plan| !plan.typed_queries.is_empty())
                .unwrap_or(false);
            if !intent_ok {
                *buckets.entry("intent_miss".to_string()).or_insert(0) += 1;
            }
            ...
            if !archive_ok {
                *buckets
                    .entry("archive_context_miss".to_string())
                    .or_insert(0) += 1;
            }
            ...
            memory_session.add_message("user", "This project repository is AxiomMe")?;
            ...
            if commit.memories_extracted < 6 {
                *buckets
                    .entry("memory_category_miss".to_string())
                    .or_insert(0) += 1;
            }
