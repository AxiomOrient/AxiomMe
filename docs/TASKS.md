...
| OMV2-004 | P0 | DONE | canonical thread identity 리뉴얼 | grouping/selection/persistence 동일 canonical id 사용 | 완료: `crates/axiomme-core/src/om/thread_identity.rs`(resolver 추가), `session/om/observer/{response.rs,threading.rs}`와 `client/search/mod.rs` canonical resolver 통합, `state/migration.rs` canonical id 규칙 통일, `client/search/backend_tests.rs::fetch_session_om_state_thread_scope_uses_scope_canonical_thread_id` 추가, `cargo test -p axiomme-core client::search::backend_tests::fetch_session_om_state_thread_scope_uses_scope_canonical_thread_id -- --exact`, `cargo test -p axiomme-core om::thread_identity::tests::thread_scope_prefers_scope_key_thread_id -- --exact`, `cargo test -p axiomme-core session::om::tests::parse_llm_multi_thread_observer_response_aggregates_primary_thread_metadata -- --exact` |
...
## Next Actions
- [NX-001] source:task priority:P0 status:todo action:OMV2-011 v1 코드/스키마/테스트 제거 전수 정리 evidence:docs/IMPLEMENTATION-PLAN.md
- [NX-002] source:task priority:P0 status:todo action:OMV2-012 Gate A 인증(OMV2-001~005 완료 스냅샷 기준) evidence:docs/IMPLEMENTATION-PLAN.md
- [NX-003] source:task priority:P1 status:todo action:OMV2-006 entry 기반 observation/reflection 모델로 전환 evidence:docs/IMPLEMENTATION-PLAN.md
- Selected For Next: NX-001, NX-002, NX-003

## NX -> TASK-ID Mapping
- NX-001 -> OMV2-011 (TODO)
- NX-002 -> OMV2-012 (TODO)
- NX-003 -> OMV2-006 (TODO)
